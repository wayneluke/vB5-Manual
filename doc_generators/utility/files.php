<?php
$root='/mnt/c/Users/wayne/Server/public_html/vbulletin5/manual/user/pages';
...
$fileCount =0;
ob_start();
echo '# vBulletin 5 Connect File List' . PHP_EOL . PHP_EOL;
echo 'MD Files with less than 200 bytes should be considered incomplete.' . PHP_EOL . PHP_EOL;
foreach($it as $file)
{
    $size ="";
    $complete= '[x]';
    ...
    if ($file->isdir()) {
        ...
        if ($file->getExtension() == 'md') {
            ...
            $size = $file->getSize();
            $size = (isset($size) AND $size > 200) ? " (" . $size . " bytes)" : "";
            $complete = $size !== "" ? "[x]":"[ ]";
            $fileCount++;
        } else {
            continue;
...
echo '- Total Articles: ' . $fileCount . PHP_EOL;
